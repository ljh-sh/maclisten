...
do_build() {
    ...
    local os="$2"
    local arch_name="$3"
    local bin_name="${BIN_NAME:-maclisten}"
    ...
    # 2. Remove build-machine-specific RPATH (privacy + reproducibility, see MCL-rel-010)
...
package_target() {
    ...
    local tarball="$OUT_DIR/maclisten-darwin-${arch_name}.tar.xz"
    ...
    # Deterministic tarball (bsdtar on macOS). Internal layout: bin/maclisten
    (cd "$pkg_dir" && \
        tar -cJf "$OUT_DIR/maclisten-darwin-${arch_name}.tar.xz" \
            --format ustar \
            bin)
...
EOF
    ...
    # SHA256SUMS
    (cd "$OUT_DIR" && shasum -a 256 maclisten-*.tar.xz BUILD_INFO.txt > SHA256SUMS 2>/dev/null || true)
...
verify_reproducibility() {
    ...
    shasum -a 256 "$OUT_DIR/$target/bin/maclisten" > /tmp/maclisten-verify-h1.txt

    do_build "$arch" "darwin" "${target#darwin-}"
    shasum -a 256 "$OUT_DIR/$target/bin/maclisten" > /tmp/maclisten-verify-h2.txt

    if diff /tmp/maclisten-verify-h1.txt /tmp/maclisten-verify-h2.txt >/dev/null; then
        echo "✓ PASS: same sha256 across two consecutive builds"
        cat /tmp/maclisten-verify-h1.txt
    else
        echo "✗ FAIL: sha256 differs"
        diff /tmp/maclisten-verify-h1.txt /tmp/maclisten-verify-h2.txt
        exit 1
    fi

    # RPATH leak check
    if otool -l "$OUT_DIR/$target/bin/maclisten" | grep -A2 "LC_RPATH" | grep -q "/Library/Developer"; then
        echo "✗ FAIL: build-machine RPATH leaked"
        otool -l "$OUT_DIR/$target/bin/maclisten" | grep -A2 "LC_RPATH" | grep "/Library/Developer"
        exit 1
    fi
    echo "✓ PASS: no build-machine RPATH"

    # User path leak check
    if strings "$OUT_DIR/$target/bin/maclisten" | grep -qE "^/Users/|^/home/"; then
...
show_usage() {
    ...
    echo "  BIN_NAME          Binary name (default: maclisten)"
    echo ""
    echo "Commands:"
